...
<Callout type='info' emoji='ℹ️'>
  You can enable darkmode by passing in `theme="dark"` prop to the
  `PaymentOverlay` component.
</Callout>

```ts copy
import React from 'react';
import { PaymentOverlay, usePayment } from "@pandabase/react-elements";

const MyPage: React.FC = () => {
  const { overlaySession, state } = usePayment();

  const handleCheckout = async () => {
    await overlaySession({
      line_items: [
        { id: "pt_...", quantity: 1 },
        { id: "pt_...", quantity: 2 }
      ]
