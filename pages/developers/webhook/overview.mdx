...
const secret = 'wh_'; // paste your secret here.
const eventData = { event: req.body, timestamp: req.headers['X-Pandabase-Timestamp'] };
...
#[post("/")]
async fn index(req_body: web::Bytes, timestamp: actix_web::web::Header, signature: actix_web::web::Header) -> impl Responder {
    let secret = b"wh_"; // paste your secret here.
...
post '/' do 
    ...
    timestamp = request.env['HTTP_X_Pandabase_Timestamp'] 
    signature = request.env['HTTP_X_Pandabase_Signature']
