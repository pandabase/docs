...
Log in to your account and navigate to the "API Tokens" section. Click the "Create New Token" button. The generated V3 API token will look similar to this: `sk_v3_xxxx`. Copy and securely store the API token.

Next in your shop, create a new webhook. The webhook secret will look like this: `wh_sk_xxx`. Copy and securely store the webhook secret.

You'll also need your Storefront ID for the `X-Storefront-Id` header.

We are storing these secrets as:

```env .env
PANDABASE_API_TOKEN="sk_v3_xxxx"
PANDABASE_WEBHOOK_SECRET="wh_sk_xxx"
PANDABASE_STOREFRONT_ID="your_storefront_id"
```
...
async function createPaymentSession(
  ...
  try {
    const response = await fetch(
      "https://pandabase.io/api/payments",
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${process.env.PANDABASE_API_TOKEN}`,
          "Content-Type": "application/json",
          "X-Storefront-Id": process.env.PANDABASE_STOREFRONT_ID!,
        },
        body: JSON.stringify({
          amount,
          currency: "USD",
    ...
    if (!result.ok) {
      throw new Error(result.error?.message || "Failed to create payment session");
    }
...
app.post("/webhook", validateSignature, (req, res) => {
  ...
  if (webhook.type === "payment.succeeded") {
    ...
    const username = payment.display_ui?.fields?.find((f: any) => f.name === "username")?.value;
    const paymentId = payment.id;

    if (username) {
      grantAccess(username, paymentId);
    }
  }
...
function grantAccess(username: string, id: string) {
  // Implement your logic here (e.g., Discord role assignment, database update, etc.)
