...
We provide fair rate limits that are designed to ensure the needs of our users. These limits are set at levels that are typically very difficult to exceed during normal usage.

The account-based rate limiter is typically more strict and dynamic compared to the global one. However, even these limits are set at levels that most users will find more than enough for their needs. Due to the potential for changes in our rate limits over time, we strongly recommend implementing a retry algorithm.
